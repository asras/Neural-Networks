...
with open(datafile, newline='', encoding='utf-8') as f:
	...
	sentences = ['%s %s %s' % (sentence_start_token, x, sentence_end_token)
	 ...
	 #Remove some of the contaminated sentences
	BadSentenceStarts = ['[ebay]', '](http',  '&', '^|', '.', '**', '*', '](https', '^^^', '[', '^']
	BadSentenceStarts = [sentence_start_token + ' ' + s for s in BadSentenceStarts]

	for i, s in enumerate(sentences):
		startsBadly = False
		for start in BadSentenceStarts:
			startsBadly = startsBadly or s.startswith(start)
		if startsBadly:
			del sentences[i]
t2 = time.time()
...
name = name + str(np.random.random())
filename = 'LSTM' + name + '.csv'
with open(filename, 'w', newline='', encoding='utf-8') as f:
	writer = csv.writer(f)
	for j, sent in enumerate(tokenized_sentences):
		writer.writerow(tokenized_sentences[j])
np.savez('LSTMwordtofromindex.txt', wtoi = word_to_index, itow = index_to_word)
